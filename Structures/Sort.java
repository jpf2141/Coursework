...
////////////////////////QUICKSORT///////////////////////////////
     ...
     */
    private static <AnyType extends Comparable<? super AnyType>>
    void quicksort( AnyType [ ] a, int left, int right )    //we dont return the array because we dont want a copy of the
                                                            //array every time, we are just changing it
    {
        if( left + CUTOFF <= right )
        {
            AnyType pivot = median3( a, left, right );  //finds median of first, last, & middle element

                // Begin partitioning
            int i = left, j = right - 1;
            for( ; ; )  //infinite loop , keeps going until something in the loop stops it (ie break)
            {
                while( a[ ++i ].compareTo( pivot ) < 0 ) { }    //increment i while it is on the "correct" side of partition
                while( a[ --j ].compareTo( pivot ) > 0 ) { }    //increment j while it is on the "correct" side of partition
                if( i < j )
